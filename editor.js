...
$('edit-actor').onclick=()=>openActor(selected);$('new-hero').onclick=()=>openActor(null,true);$('new-monster').onclick=()=>openActor(null,false);
...
const originalRender=render;render=function(){originalRender();toolsBar.hidden=view!=='mj';$('owner').replaceChildren();actors.forEach((a,i)=>{if(a.hero)$('owner').add(new Option(a.name,String(i)))});$('owner').value=String(owner);const a=actors[selected];$('actor-notes').textContent=a.notes||'';attackSelect.replaceChildren();a.attacks.forEach((at,i)=>attackSelect.add(new Option(at.name,String(i))));attackSelect.value=String(a.activeAttack||0);attackSelect.hidden=!a.attacks.length||!!equippedPool(a,catalog.items);document.querySelectorAll('.token').forEach((t,i)=>{if(actors[i].image){t.replaceChildren();const im=document.createElement('img');im.src=actors[i].image;im.alt='';t.append(im)}});scheduleSave()};
