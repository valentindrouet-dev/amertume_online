...
## Sauvegarde
IndexedDB local conserve scène, combattants, catalogue modifié et images validées. Un message signale l’échec si le stockage est indisponible ou plein. L’effacement des données du site supprime cette sauvegarde. Depuis la v0.178, l’onglet **Paramètres** propose une **sauvegarde globale** : « Exporter toute la partie » télécharge un fichier `amertume-AAAAMMJJ-HHMM.json` qui contient aventuriers, adversaires, bestiaire, armurerie, talents, cartes (images comprises) et scène en cours ; « Importer une sauvegarde » (vue MJ) remplace la partie du navigateur par ce fichier après confirmation, puis la republie si la publication automatique est active. Le fichier est la sauvegarde locale telle quelle : les formes 7 et 8 sont acceptées. La publication Firebase (v0.08) partage le contenu du MJ, mais la partie jouée reste locale : ni synchronisation des dés, des déplacements ou des PV, ni compte joueur. Les vues MJ/joueur sont des interfaces locales et ne constituent pas une sécurité d’accès.
